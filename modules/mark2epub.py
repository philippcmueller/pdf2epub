...
def get_TOC_XML(default_css_filenames, markdown_filenames):
    ...
    for i, md_filename in enumerate(markdown_filenames):
        toc_xhtml += """<li><a href="s{:05d}-{}.xhtml">{}</a></li>""".format(i, md_filename.split(".")[0],
                                                                             md_filename.split(".")[0])
    toc_xhtml += """</ol>\n</nav>\n</body>\n</html>"""
...
def get_chapter_XML(work_dir: str, md_filename: str, css_filenames: list[str], content: Optional[str] = None) -> tuple[
    str, list[str]]:
    """
...
def convert_to_epub(markdown_dir: Path, output_path: Path, metadata: dict = None) -> None:
    """
    ...
    if not metadata:
        raise ValueError("Metadata is required for EPUB conversion")
    ...
    print("Writing metadata to description.json...")
    # Create description.json with provided metadata
    description_path = markdown_dir / "description.json"

    # Convert metadata format to match what the EPUB generator expects
    print(metadata)
    description_data = {
        "metadata": {
...
def main(args):
    ...
    # Get current folder name and set as doc ID
    doc_id = os.path.basename(work_dir)
    ...
    try:
        ...
        metadata_path = os.path.join(work_dir, f'{doc_id}_metadata.json')
        ...
        if os.path.exists(metadata_path):
            ...
            raise FileNotFoundError(f"Description file not found: {metadata_path}")
        ...
        print(f'Updated metadata: {description_json}')

        # Review markdown files and store updated content
        chapter_contents = {}
        for chapter in description_json["chapters"]:
        ...
        title = description_json["metadata"].get("dc:title", "Untitled Document")
        authors = description_json["metadata"].get("dc:creator", None)

        # Compile list of files
        all_md_filenames = []
        all_css_filenames = description_json["default_css"][:]
        for chapter in description_json["chapters"]:
        ...
        for i, chapter in enumerate(description_json["chapters"]):
            css_files = description_json["default_css"][:]
            if chapter["css"]:
        ...
        with zipfile.ZipFile(output_path, "w") as epub:
            ...
            epub.writestr("OPS/package.opf",
                          get_packageOPF_XML(
                              ...
                              description_data=description_json
                          ),
            ...
            for i, chapter in enumerate(description_json["chapters"]):
                print(f"  Writing chapter {i + 1}/{len(description_json['chapters'])}: {chapter['markdown']}")
                epub.writestr(
            ...
            epub.writestr("OPS/TOC.xhtml",
                          get_TOC_XML(description_json["default_css"], all_md_filenames),
                          zipfile.ZIP_DEFLATED
                          )
